 ...
 */
function mdspringspray_install_tasks(&$install_state) {

  $tasks = array();

  // Add our custom CSS file for the installation process
  drupal_add_css(drupal_get_path('profile', 'mdspringspray') . '/mdtheme.css');
 ...
 */
function mdspringspray_form_install_configure_form_alter(&$form, $form_state) {
  $form['site_information']['site_name']['#default_value'] = 'MD Springspray';
  $form['#submit'][] = 'mdspringspray_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function mdspringspray_install_configure_form_submit($form, &$form_state) {
  global $user;
  $sql_file = dirname(__FILE__) . '/mdspringspray.sql';
  $count = mdspringspray_import_sql($sql_file);
...
function mdspringspray_import_sql($filename) {
  global $databases;
  $databasesName = array(
    "pgsql" => "PostgreSQL",
    "sqlite" => "SQLite"
  ...
  if ($file) {
    ...
    if ($connect) {
      while (!feof($file)) {
        ...
        if (preg_match('|;$|', $line)) {
          try {
            $connect->exec(mdspringspray_prefixTables($sql_command));
...
function mdspringspray_prefixTables($sql) {
  global $databases;
